...
export function PRCard({ pr }: Props) {
  ...
  const lagHours =
    pr.firstReviewAt != null
      ...
      : null;
  return (
    <div className="card p-3.5 hover:border-border-strong transition-colors">
      {/* top row */}
      <div className="flex items-start gap-2.5 mb-2.5">
        <div className="w-7 h-7 rounded-full bg-surface-subtle flex items-center justify-center text-[10px] font-medium text-text-muted shrink-0">
          {initials(pr.author)}
        </div>
        ...
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="text-[11px] text-text-subtle hover:text-text-secondary px-1.5 py-0.5 rounded border border-border-default shrink-0 transition-colors"
        >
          GH ↗
        </a>
      </div>
      ...
      {/* footer */}
      <div className="flex items-center gap-4 border-t border-border-subtle pt-2">
        ...
        <span className="ml-auto text-[11px]">
          <span className="text-status-success-dot">+{pr.additions}</span>
          <span className="text-text-disabled"> / </span>
          <span className="text-status-danger-dot">-{pr.deletions}</span>
        </span>
      </div>
    </div>
  );
