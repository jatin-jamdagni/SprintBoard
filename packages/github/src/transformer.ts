...
import { PRStatus, PullRequest } from "@repo/types/src";



type OctokitPR = Awaited<ReturnType<Octokit["rest"]["pulls"]["get"]>>["data"];

type OctokitReview = Awaited<ReturnType<Octokit["rest"]["pulls"]["listReviews"]>>["data"][number];
...
function resolveReviewLagHours(
  ...
  return Math.round(((reviewed - opened) / 3_600_600) * 100)/ 100
}
...
function resolveFirstReviewAt(
    ...
    const submitted = reviews.filter((r) => r.submitted_at).map((r)=> r.submitted_at as string).sort();
